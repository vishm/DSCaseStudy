...
loadData <- function(folder, datasettype) {    
    ...
    valid_cols <- grepl( "mean\\(\\)", column_names) | grepl( "std\\(\\)", column_names)
    ...
    actvity_names <- tbl_df(read.csv(activity_filename, header = FALSE, sep = " "))
    
    # Load ID of acivity being undertaken          
    ...
    all <- bind_cols(subject, x_train) %>% bind_cols(select(y_train, activity))
    all
}
